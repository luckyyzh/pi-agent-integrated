...
test("managed runtime never enables project-local resources", async (t) => {
  const { cwd, agentDir } = await createProjectFixture(t);
  await mkdir(join(cwd, ".pi", "extensions"), { recursive: true });
  const previous = process.env.PI_AGENT_MANAGED_RUNTIME;
  process.env.PI_AGENT_MANAGED_RUNTIME = "1";
  t.after(() => {
    if (previous === undefined) delete process.env.PI_AGENT_MANAGED_RUNTIME;
    else process.env.PI_AGENT_MANAGED_RUNTIME = previous;
  });
...
test("all project resource loaders and reloads enforce project trust", async () => {
  ...
  assert.match(pluginsSource, /createAppSettingsManager\(cwd, agentDir, projectTrust\.trusted\)/);
  assert.match(
    skillsInstallSource,
    /getProjectTrustStatus\(cwd, getAgentDir\(\)\)\.trusted/,
  );
  assert.equal(
    Array.from(pluginsSource.matchAll(/createAppSettingsManager\([^\n]+projectTrust\.trusted\)/g)).length,
    2,
  );
...
test("the trust API invalidates cached models and restricted runtimes", async () => {
  ...
  assert.match(rpcSource, /trackStartingSession\(cwd\)/);
