...
// Mirrors Next's [...path] catch-all: split the raw pathname first, then
// percent-decode each segment individually.
function segmentsFromRequestUrl(url: string): string[] {
  const pathname = new URL(url).pathname;
  const suffix = pathname.startsWith("/api/files/") ? pathname.slice("/api/files/".length) : "";
...
async function getWritableFile(
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    return { response: Response.json({ error: "Not a writable file" }, { status: 403 }) };
  }
...
export function registerFileRoutes(app: Hono): void {
  app.post("/api/files/*", async (c) => {
    ...
    try {
      const segments = segmentsFromRequestUrl(c.req.raw.url);
      ...
      if (type === "write") {
        ...
        if (readonlyFile) {
          return c.json({ error: "This file type cannot be edited as text" }, 400);
        }
      ...
      if (type === "upload-check") {
        const body = await c.req.json().catch(() => null) as { fileNames?: unknown } | null;
        const fileNames = parseUploadFileNames(body?.fileNames);
        if (!fileNames) {
          return c.json({ error: "fileNames must be an array of strings" }, 400);
        }
        const validationError = validateUploadFileNames(fileNames);
        if (validationError) {
          return c.json({ error: validationError }, 400);
        }
        return c.json(inspectUploadTargets(directory, fileNames));
      }

      if (type !== "upload") {
        return c.json({ error: "Invalid upload request type" }, 400);
      }

      const strategy = parseUploadConflictStrategy(c.req.query("conflict") ?? null);
      if (!strategy) {
        return c.json({ error: "Invalid conflict strategy" }, 400);
      }

      let formData: FormData;
      try {
        formData = await parseFormDataWithinLimit(c.req.raw, MAX_UPLOAD_REQUEST_BYTES);
      } catch (error) {
        if (error instanceof RequestBodyTooLargeError) {
          return c.json({ error: "Uploads must total 100MB or less" }, 413);
        }
        throw error;
      }
      ...
      if (files.some((file) => file.size > MAX_UPLOAD_FILE_BYTES)) {
        return c.json({ error: "Each upload must be 25MB or smaller" }, 413);
      }
  ...
  app.get("/api/files/*", async (c) => {
    try {
      const segments = segmentsFromRequestUrl(c.req.raw.url);
      const filePath = filePathFromSegments(segments);
      const rawType = c.req.query("type") ?? "list";
      const type = parseFileRequestType(rawType);
      if (!type) {
        return c.json({ error: "Invalid file request type" }, 400);
      }
      const sessionId = c.req.query("sessionId") ?? null;
      ...
      if (type === "read") {
        ...
        if (imageMime) {
          if (stat.size > IMAGE_PREVIEW_MAX_BYTES) {
            return c.json({ error: "Image too large (>10MB)" }, 413);
          }
          return streamFile(filePath, stat, imageMime, c.req.raw.headers.get("range"));
        }
        const audioMime = getAudioMime(filePath);
        if (audioMime) {
          return streamFile(filePath, stat, audioMime, c.req.raw.headers.get("range"));
        }
        const documentMime = getDocumentMime(filePath);
        if (documentMime) {
          return streamFile(filePath, stat, documentMime, c.req.raw.headers.get("range"));
        }
        if (stat.size > TEXT_PREVIEW_MAX_BYTES) {
          return c.json({ error: "File too large for preview (>256KB)" }, 413);
        }
        const content = fs.readFileSync(filePath, "utf-8");
        const language = getLanguage(filePath);
        return c.json({ content, language, size: stat.size });
      }
      ...
      if (type === "download") {
        ...
        return streamFile(filePath, stat, mime, c.req.raw.headers.get("range"), true);
      }
      ...
      if (type === "preview") {
        ...
        if (stat.size > DOCX_PREVIEW_MAX_BYTES) {
          return c.json({ error: "DOCX too large for preview (>10MB)" }, 413);
        }
